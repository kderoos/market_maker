...
// fn apply_execution_OG(state: &mut PositionState, exec: &ExecutionEvent, contract_type: &ContractType) {
//     match exec.side {
//         OrderSide::Buy => {
...
//                     realized_pnl(exec.price, state.avg_price, closed, contract_type.clone());

//             }

//             state.position = new_pos;
//             // state.cash -= cost;
//             state.cash += trade_cash_delta(exec.price, exec.size, exec.side, contract_type.clone());
...
//                 let closed = (-signed).min(state.position);
//                 state.realized_pnl +=
//                     realized_pnl(state.avg_price, exec.price, closed, contract_type.clone());
//             }

//             state.position = new_pos;
//             // state.cash += proceeds;
//             let delta = trade_cash_delta(exec.price, exec.size, exec.side, contract_type.clone());
//             state.cash += delta;
...
//     }
// }
// fn trade_cash_delta(price: f64, size: i64, side: OrderSide, ct: ContractType) -> f64 {
//     let signed = match side {
//         OrderSide::Buy => size as f64,
//         OrderSide::Sell => -(size as f64),
//     };

//     match ct {
//         ContractType::Linear => {
//             // USDT per contract = price
//             -price * signed
//         }
//         ContractType::Inverse => {
//             // USDT per contract = 1 / price
//             -(signed) / price
//         }
//     }
// }
// fn realized_pnl(entry: f64, exit: f64, closed: i64, ct: ContractType, fee: f64) -> f64 {
//     let c = closed as f64;
//     match ct {
//         ContractType::Linear => (exit - entry) * c - fee * (entry + exit),
//         ContractType::Inverse => c * (1.0 / entry - 1.0 / exit) - fee * (1.0 / entry + 1.0 / exit),
//     }
// }
